...
> ## ⚠️ 现状（2026-09-11）：这条路线已判定不可行
>
> 有账号在**尚未使用本功能**的情况下被封。复盘结论：改包必然改变「包体指纹 + 证书指纹」，
> 而热更模块的 `ZipPrint.getApkHash`（APK zip 条目指纹）与自签证书这两条都**改不掉**；
> 检测点在你装完第一次启动时就已触发 —— 与"用没用过扫描"无关。
> 另有 native 侧易盾环境检测（`libsec-lib.so` / `libenvsdk.so`）未也无法触碰。
>
> **归因已闭环（实验 B，2026-09-11）**：干净非 root 机 + 新账号 + 本包，玩几把**仍然被封**
> → 排除账号历史、排除设备环境，**结论落在"改包"本身**。
> 所以直装版（同一套改动、只是包名不同）同样必封，**不需要再花一个账号去验证**。
> **`out/` 下的两个 APK 请勿再安装、勿再分发。**
>
> **完整证据链、A/B/C 归因实验、止损清单与仅存的方向（root + 不改包 + `process_vm_readv`）
> 见 [`docs/BAN_POSTMORTEM.md`](docs/BAN_POSTMORTEM.md)。**
